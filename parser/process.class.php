<?php
	...
	class Process {
		...
		public function run() {
			...
			// Relay DB Connect
			MSSQL::init();
			// Now fetch corresponding Relay userId and presId from Relay's DB
			$this->fixMissingRelayUserIds();
			// Close conn
			$this->done();
		}
		...
		private function parse() {
			...
			foreach($logfiles as $file_name) {
				...
				foreach($presentations['hits'] as $path => $ips) {
					...
					MySQL::updateHitsTable($path, $hits, $timestamp, $username);
				}
				...
				MySQL::updateDailyTable($presentations['daily']);
				// Done with this file
			...
			MySQL::updateLastLogfileRead($last_logfile_read);
			// And update our timestamps
			MySQL::updateFirstRecordTimestamp($first_record_timestamp);
			MySQL::updateLastRecordTimestamp($last_record_timestamp);
			//
		...
		private function getLogFilesLatest($logfiles) {
			...
			if(empty($last_logfile_read = MySQL::getLastLogfileRead())) {
		...
		/**
		 * Scan table for paths with no IDs for user/presentation, and for each of these
		 * call Relay DB to fetch these IDs and add to table. Having a user ID (in particular)
		 * is very useful, since a username may change (e.g. 'fusjonering').
		 */
		private function fixMissingRelayUserIds() {
			// Get all paths with no user id
			$missingIdsArr = MySQL::getPathsWithNoUserId();
			foreach($missingIdsArr as $index => $presObj) {
				if(isset($presObj['path'])) {
					// userId and presId
					$presIds = MSSQL::getIdsForPath($presObj['path']);
					// insert into hits table
					MySQL::updateHitsTableWithIDs($presObj['path'], $presIds['presUser_userId'], $presIds['presId']);
				}
			}
		}
